...
from .Rom import ROM_PLAYER_LIMIT as SMZ3u_ROM_PLAYER_LIMIT
...
# SMZ3u
SMZ3u_ROMNAME_START = ROM_START + 0x00FFC0
ROMNAME_SIZE = 0x15

SAVEDATA_START = WRAM_START + 0xF000

SMZ3u_INGAME_MODES = {0x07, 0x09, 0x0B}
ENDGAME_MODES = {0x19, 0x1A}
SM_ENDGAME_MODES = {0x26, 0x27}
SMZ3u_DEATH_MODES = {0x15, 0x17, 0x18, 0x19, 0x1A}

SMZ3u_RECV_PROGRESS_ADDR = SRAM_START + 0x4000         # 2 bytes
SMZ3u_RECV_ITEM_ADDR = SAVEDATA_START + 0x4D2          # 1 byte
SMZ3u_RECV_ITEM_PLAYER_ADDR = SAVEDATA_START + 0x4D3   # 1 byte


class SMZ3uSNIClient(SNIClient):
    game = "SMZ3u"
    patch_suffix = ".apsmz3u"

    async def validate_rom(self, ctx):
        from SNIClient import snes_buffered_write, snes_flush_writes, snes_read

        rom_name = await snes_read(ctx, SMZ3u_ROMNAME_START, ROMNAME_SIZE)
        if rom_name is None or rom_name == bytes([0] * ROMNAME_SIZE) or rom_name[:3] != b"ZSM":
            return False

        ctx.smz3u_new_message_queue = rom_name[7] in b"1234567890"
        ctx.game = self.game
    ...
    async def game_watcher(self, ctx):
        ...
        if ctx.smz3u_new_message_queue:
        ...
        data = await snes_read(ctx, SMZ3u_RECV_PROGRESS_ADDR + send_progress_addr_ptr_offset, 4)
        if data is None:
            return
        ...
        while (recv_index < recv_item):
            ...
            message = await snes_read(ctx, SMZ3u_RECV_PROGRESS_ADDR + send_progress_addr_table_offset + item_address, send_progress_size)
            ...
            recv_index += 1
            snes_buffered_write(ctx, SMZ3u_RECV_PROGRESS_ADDR + send_progress_addr_ptr_offset, bytes([recv_index & 0xFF, (recv_index >> 8) & 0xFF]))

            from .TotalSMZ3.Location import locations_start_id
            from . import convertLocSMZ3uIDToAPID
            location_id = locations_start_id + convertLocSMZ3uIDToAPID(item_index)
        ...
        data = await snes_read(ctx, SMZ3u_RECV_PROGRESS_ADDR + recv_progress_addr_ptr_offset, 4)
        if data is None:
            return
        ...
        if item_out_ptr < len(ctx.items_received):
            ...
            player_id = item.player if item.player < SMZ3u_ROM_PLAYER_LIMIT else 0
            snes_buffered_write(ctx, 
                                SMZ3u_RECV_PROGRESS_ADDR + item_out_ptr * recv_progress_size,
                                bytes([player_id, item_id]) if ctx.smz3u_new_message_queue else
                                bytes([player_id & 0xFF, (player_id >> 8) & 0xFF, item_id & 0xFF, (item_id >> 8) & 0xFF]))
            item_out_ptr += 1
            snes_buffered_write(ctx, SMZ3u_RECV_PROGRESS_ADDR + recv_progress_addr_table_offset, bytes([item_out_ptr & 0xFF, (item_out_ptr >> 8) & 0xFF]))
